...
class TestReportsGenerator(BaseDbTestCase):
    ...
    def test_report_2(self):
        ...
        cursor.execute("""INSERT INTO `bids` (`original_id`, `tender_id`, `status_id`) VALUES ("{}", {}, 1)""".format( tender_id, tender_db_id))
        bid_db_id = cursor.lastrowid
        edr_status = 1
        for i in range(30):
            if edr_status == 1:
                cursor.execute("""INSERT INTO `tenderers` (`identifier`, `scheme`, `edr_status`) VALUES ({}, "UA-EDR", 1)""".
                               format(edr_ids[i]))
                edr_status = 0
            else:
                cursor.execute("""INSERT INTO `tenderers` (`identifier`, `scheme`, `edr_status`) VALUES ({}, "UA-EDR", 0)""".
                               format(edr_ids[i]))
                edr_status = 1
            cursor.execute("""INSERT INTO `tenderers_bids` (`tenderer_id`, `bid_id`) VALUES ({}, {})""".format(cursor.lastrowid, bid_db_id))
        cursor.close()
        self.conn.commit()
    ...
    def test_report_3(self):
        ...
        cursor.close()
        self.conn.commit()
        rep_gen = GeneratorOfReports('01.05.2016', '01.06.2018', '3', 'test', 'test', test_config)
        self.assertEqual(rep_gen.report_number, '3')
        self.assertEqual(rep_gen.password, self.password)
        self.assertEqual(rep_gen.config, test_config)
        self.assertEqual(rep_gen.ws[2][1].value, u'1.00')
